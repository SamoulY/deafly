...
extern "C" void memory_advance(
 int n,const uint32_t* ptr,const int32_t* post,const int16_t* contact,
 ...
 int nplastic,const uint32_t* plastic_edge,const int32_t* plastic_pre,
 ...
 float* adaptation,float adaptation_jump,float adaptation_tau,float* plastic_weight) {
   ...
   *nactive=kept;
   for(int q=0;q<queue_count[slot];q++){
     ...
     if(modulation_mask[i]){
       ...
       auto plastic_cursor=first_plastic(ptr[i]);
       for(uint32_t e=ptr[i];e<ptr[i+1];e++){
         const int j=post[e];
         modulation[j]*=std::exp(-dt*(*clock-modulation_last[j])/100.f);
         modulation[j]+=std::abs(read_weight(e,plastic_cursor))/.275f;modulation_last[j]=*clock;
       }
       if(learning_enabled && dan_index[i]>=0){
         for(int p=0;p<nplastic;p++){
           ...
           const float candidate=plastic_weight[p]*std::exp(-eta*gain*trace);
           const float lower=baseline_weight[p]*floor_fraction;
           plastic_weight[p]=candidate>lower?candidate:lower;
         }
       }
       continue;
     }
     auto plastic_cursor=first_plastic(ptr[i]);
       for(uint32_t e=ptr[i];e<ptr[i+1];e++){
       const float edge_weight=read_weight(e,plastic_cursor);
       const int j=post[e];evolve(j,*clock,drive[j]);
       if(refractory[j]==0){g[j]+=edge_weight;awaken(j);}
     }
   }
   queue_count[slot]=0;

...
export function sampleRGB(rgb,width,height,uv,channels=null,result=new Float32Array(uv.length/2)){
 if(!(rgb instanceof Uint8Array)||rgb.length!==width*height*3||uv.length%2)throw Error('Invalid RGB input');
 if(result.length!==uv.length/2)throw Error('Invalid sensory scratch shape');
...
export function buildDrive(graph,state,rgb,width,height,ms,{laminaBias=12,tonic,stimulation=[]}={}){
 ...
 const n=graph.nodes,drive=state.drive&&state.drive.length===n?state.drive:new Float32Array(n),light=sampleRGB(rgb,width,height,graph.uv,null,state.light&&state.light.length===graph.uv.length/2?state.light:undefined),r8=sampleRGB(rgb,width,height,graph.r8_uv,graph.r8_channel,state.r8Sample&&state.r8Sample.length===graph.r8_uv.length/2?state.r8Sample:undefined),rate=1-Math.exp(-Math.round(ms/.1)*.1/10);
 drive.fill(0);

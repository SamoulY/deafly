...
self.onmessage=async({data:m})=>{try{
 ...
 if(m.type==='OBSERVE'||m.type==='REINFORCE'){
  ...
  const states=[];for(const [name,a]of Object.entries(brain.arrays))if(!['post','contact','ids'].includes(name))states.push([name,await hash(new Uint8Array(a.buffer,a.byteOffset,a.byteLength))]);for(const [name,a]of Object.entries({...brain.learning,...brain.sensory}))states.push([name,await hash(new Uint8Array(a.buffer,a.byteOffset,a.byteLength))]);
  const checkpoint_hash=await hash(new TextEncoder().encode(JSON.stringify(states)));
